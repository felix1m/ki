...
#Aussagenlogik:
def logic():
    global resolutionbutton
    global showknowledgebase
    global inputgoal
    global error
    if neu:
    ...
    knowledgebase = []
    # buttons etc:
    resolutionbutton = Button(ki, text="Do Resolution",
                          command=lambda: run_resolution(knowledgebase))
    resolutionbutton.grid(row=0, column=4)
...
def run_resolution(knowledgebase):
    def getgoal():
        #enter goal, get queue "goal" with the goal as only item:
        goal=queue.LifoQueue()
        g=e1.get()
        if " " in g:
            printfenster.insert(END,"The input wasn't exactly one literal without any spaces. ")
        elif g=="":
            printfenster.insert(END, "There seems to be no goal to be achieved.\n" )
        else:
            printfenster.insert(END, "The input goal is '"+g+"'.\n" )
            if g[0]=="-":
                goal.put([g[1:]])
            else:
                goal.put(["-"+g])   
    ...
    if not error:
        ...
        #create new window to input goal
        window = Tk()
...
def show_knowledgebase(knowledgebase):
## to print clauses with clauselnames C0,C1,... and without'...'
    if not error:
        table = Tk()
        table.title("Knowledgebase")
        for cl in range(len(knowledgebase)):
            label1= Label(table,text="C"+str(cl)+"= [")
            label1.grid(row=cl,column=0)
            text=""
            for c in range(len(knowledgebase[cl])):
                if not c==len(knowledgebase[cl])-1:
                    text=text+str(knowledgebase[cl][c])+", "
                else:
                    text=text+str(knowledgebase[cl][c])
            label2= Label(table,text=text)
            label2.grid(row=cl,column=1)
            label3=Label(table,text="]")
            label3.grid(row=cl,column=2)
    else:
        printfenster.insert(END, "Cannot execute due to previous error!\n")
...
def clear_gui():
    ...
    #falls vorher aussagenlogik:
    if resolutionbutton != "":
        resolutionbutton.destroy()
        showknowledgebase.destroy()
...
submenu5 = Menu(scenariomenu)
submenu5.add_command(label="Open Knowledgebase",
                     command=logic)
scenariomenu.add_cascade(label="Logic", menu=submenu5)
